...
def train_cat_mouse(num_episodes = 25):
    n_agents = 2
    n_mice = 2
    env = CatMouseMA(n_agents=n_agents, n_mice=n_mice, render_mode='human') #render_mode='human'_agents+3*n_mice, action_dim=1, num_agents=num_agents, env=env)
    agents = []
    for i in range(n_agents):
        agents.append(local_agent.Agent(n_agents, n_mice, i)) 
    ...
    for ep in range(num_episodes):
        ...
        next_state, communication, reward, terminated, info = env.step([random.random(), random.random()])
        if terminated:
            break
        env.render()
