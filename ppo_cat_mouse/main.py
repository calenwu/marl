...
def main():
	...
	for n_epi in range(10000):
		s, info = env.reset()
		s = np.array(trans_state(env.get_global_obs()))

		for t in range(T_horizon):
			prob = model.pi(torch.tensor(s, dtype=torch.float32))
			m = Categorical(prob)
			a = m.sample().item()
			a_n = np.array([get_action(x) for x in d[a]])
			s_prime, r, done, _, info = env.step(a_n)
			s_prime = trans_state(env.get_global_obs())

			model.put_data((s, a, sum(r), s_prime, prob[a].item(), done))
			s = s_prime
